...
class ElasticCloud(VectorDB):
    ...
    def _create_indice(self, client) -> None:
        ...
        settings = {
            'index' : {
                'number_of_shards': psutil.cpu_count(logical=False)
            }
        }
    ...
    def insert_embeddings(
        ...
        **kwargs,
        ...
        try:
            for success, info in parallel_bulk(self.client, insert_data, thread_count=psutil.cpu_count(logical=False)):
                if not success:
    ...
    def optimize(self):
        ...
        while True:
            ...
            if task_status['completed']:
                return
        shard_segs = client.indices.segments()["indices"]["vdb_bench_indice"]["shards"]
        segs = sum(s[0]["num_committed_segments"] for s in shard_segs.values())
        log.info(f"Segment count: {segs}")
