...
class PgVector2(VectorDB):
    ...
    def __init__(
        ...
        collection_name: str = "PgVector2Collection",
        drop_old: bool = False,
        **kwargs,
    ):
        ...
        if drop_old and self.table_name in pq_metadata.tables:
            log.info(f"Pgvector2 client drop table : {self.table_name}")
    ...
    def _create_index(self, pg_engine):
        ...
        index = Index(self._index_name, self.pg_table.c.embedding,
            ...
            postgresql_with={'M': index_param["M"],  
            'ef_Construction': index_param["efConstruction"]},
    ...
    def _create_table(self, dim, pg_engine : int):
        try:
            ...
            log.warning(f"Failed to create pgvector2 table: {self.table_name} error: {e}")
            raise e from None
    ...
    def insert_embeddings(
        ...
        **kwargs: Any,
        ...
        try:
            ...
            log.warning(f"Failed to insert data into pgvector2 table ({self.table_name}), error: {e}")   
            return 0, e
    ...
    def search_embedding(        
        ...
        with self.pg_engine.connect() as conn: 
            conn.execute(text(f'SET hnsw.ef_search = {search_param["ef"]}'))
            conn.commit()
