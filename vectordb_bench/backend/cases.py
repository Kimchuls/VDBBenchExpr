...
class Performance128D1M(PerformanceCase):
    ...
    description: str = """This case tests the search performance of a vector database with a medium dataset (<b>SIFT 1M vectors</b>, 128 dimensions) at varying parallel levels.
Results will show index building time, recall, and maximum QPS."""
    load_timeout: float | int = config.LOAD_TIMEOUT_768D_1M
    optimize_timeout: float | int | None = config.OPTIMIZE_TIMEOUT_768D_1M
...
class Performance128D100M(PerformanceCase):
    case_id: CaseType = CaseType.Performance128D100M
    dataset: DatasetManager = Dataset.SIFT1M.manager(100_000_000)
...
class Performance96D10M(PerformanceCase):
    ...
    dataset: DatasetManager = Dataset.DEEP1M.manager(10_000_000)
    name: str = "Search Performance Test (10M Dataset, 96 Dim)"
    description: str = """This case tests the search performance of a vector database with a large dataset (<b>DEEP 10M vectors</b>, 96 dimensions) at varying parallel levels.
Results will show index building time, recall, and maximum QPS."""
    load_timeout: float | int = config.LOAD_TIMEOUT_768D_1M
    optimize_timeout: float | int | None = config.OPTIMIZE_TIMEOUT_768D_1M
